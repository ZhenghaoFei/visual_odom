...
Mat loadImageLeft(int timestep){
    char filename[200];
    sprintf(filename, "/Users/holly/Downloads/KITTI/sequences/00/image_0/%06d.png", timestep);
...
Mat loadImageRight(int timestep){
    char filename[200];
    sprintf(filename, "/Users/holly/Downloads/KITTI/sequences/00/image_1/%06d.png", timestep);
    Mat image = imread(filename);
    cvtColor(image, image, COLOR_BGR2GRAY);
...
void visualOdometry(int current_timestep, Mat& rotation, Mat& translation)
{
    std::cout << "Timestep " << current_timestep << std::endl;

    // load images
    Mat image_left_t0 = loadImageLeft(current_timestep);
    Mat image_right_t0 = loadImageRight(current_timestep);

    Mat image_left_t1 = loadImageLeft(current_timestep+1);
    Mat image_right_t1 = loadImageRight(current_timestep+1);
...
void integrateOdometry(Mat& pose, Mat& Rpose, const Mat& rotation, const Mat& translation)
{
    pose = pose + Rpose * translation;
    Rpose = rotation * Rpose;
}

void display(Mat& trajectory, Mat& pose)
{
...
int main(int argc, char const *argv[])
{
    ...
    std::vector<Matrix> pose_ground_truth = loadPoses(filename_pose);
    ...
    // {
    //     visualOdometry(timestep, rotation, translation);

    //     integrateOdometry(pose, Rpose, rotation, translation);
